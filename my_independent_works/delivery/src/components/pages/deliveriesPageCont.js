...
const DeliveriesPageCont = memo((props) => {
    ...
    const onFilteredItems = (data) => {
            ...
            setFilteredDeliveries(data.filter(item => item.date === selectedDate));
        };        
   ...
   /*  const optionsForm = (data) => {
    ...
    const deliveryRender = (arr) => {
        if (arr.length === 0) {
            return <h5 className="text-center mt-5">СЕГОДНЯ ВЪЕЗД АВТОТРАНСПОРТА НЕ ПЛАНИРУЕТСЯ</h5>
        }
        return arr.map((item) => {
            return (
                <li className="list-group-item d-flex justify-content-between align-items-start" key={item.id}>
                    <div className="ms-2 me-auto">
                        ...
                        {item.operation === 'inTo' ? ` ${item.goods}` : ` забирает ${item.goods}`} {item.weight}тн{item.documents ? `, ${item.documents}` : null}</span>
                    </div>                    
                </li>
            )
        });
