...
const DeliveryForm = (props) => {
    ...
    console.log(inputAddress.current);
    console.log(state)


    const postDelivery = (values, inputAddress) => {
        const newDelivery = {
            name: values.name,
    ...
    const selectMessage = state.length < 1 || state === '' ? (<p className="error">Обязательное поле!</p>) : null;
        ...
        <Formik
            ...
            onSubmit={(values, { resetForm }) => {
                if (selectMessage === null && state !== '0') {
                    postDelivery(values, inputAddress);
